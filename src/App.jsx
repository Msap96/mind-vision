...
const VisualizationApp = () => {
  ...
  const exercises = [
    {
      ...
      description: "Imagine a blank movie screen in your mind. Practice making it bigger, smaller, closer, and further away.",
      duration: 600,
      difficulty: "Beginner",
      prompts: ["How clear was your mental screen?", "Could you adjust its size easily?"]
    },
    {
      title: "Object Visualization",
      description: "Visualize a simple object like an apple. Focus on its color, texture, and try rotating it in your mind.",
      duration: 900,
      difficulty: "Intermediate",
      prompts: ["What details could you see clearly?", "How stable was the image?"]
    },
    {
      title: "Scene Construction",
      description: "Build a peaceful scene piece by piece - start with the sky, add trees, water, and other elements gradually.",
      duration: 1200,
      difficulty: "Advanced",
  ...
  const handlePrevious = () => {
    ...
    setProgress(0);
    setTimeRemaining(exercises[(currentExercise - 1 + exercises.length) % exercises.length].duration);
    setJournalEntry('');
  };
  ...
  return (
    <div className="min-h-screen mind-vision-gradient p-4 sm:p-6 md:p-8">
      <div className="max-w-2xl mx-auto space-y-6">
        <Card className="card-hover shadow-lg">
          <CardHeader className="bg-gradient-to-r from-primary-100 to-primary-50 rounded-t-lg">
            <CardTitle className="text-3xl font-bold text-primary-700">MindVision</CardTitle>
          </CardHeader>
          <CardContent className="p-6">
            <div className="space-y-6">
              <div className="bg-primary-50 rounded-lg p-6 shadow-inner">
                <h2 className="text-xl font-semibold mb-2 text-primary-700">{currentEx.title}</h2>
                <p className="text-gray-600 mb-4 leading-relaxed">{currentEx.description}</p>
        ...
        <Card className="journal-card card-hover shadow-lg">
          <CardHeader className="bg-gradient-to-r from-primary-50 to-white">
            <CardTitle className="text-2xl text-primary-700">Reflection Journal</CardTitle>
          </CardHeader>
          <CardContent className="p-6">
            <div className="space-y-4">
              <div className="space-y-2">
                {currentEx.prompts.map((prompt, index) => (
                  <p key={index} className="text-gray-600 italic">"{prompt}"</p>
                ))}
              </div>
              <textarea
        ...
        {journalEntries.length > 0 && (
          <Card className="journal-card card-hover shadow-lg">
            <CardHeader className="bg-gradient-to-r from-primary-50 to-white">
              <CardTitle className="text-2xl text-primary-700">Previous Entries</CardTitle>
            </CardHeader>
            <CardContent className="p-6">
              <div className="space-y-4">
                {journalEntries.map((entry, index) => (
                  <div key={index} className="border-b pb-4 hover:bg-primary-50 p-3 rounded-lg transition-colors">
                    <div className="text-sm text-primary-600">{entry.date} - {entry.exercise}</div>
